...
QuadTreeNode.prototype.addObjectsInAreaToArray = function(box, array, index)
{
    index = index || 0;
    ...
    if (wade.boxIntersectsBox(this, box))
    {
        for (var i=0; i<this._objects.length; i++)
        {
            ...
            {
                array[index++] = this._objects[i];
            }
        }
        for (i=0; i<this._children.length; i++)
        {
            index += this._children[i].addObjectsInAreaToArray(box, array, index);
        }
    }
    return index - initialIndex; // how many we've added
};
