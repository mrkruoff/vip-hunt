...
function Wade_drawFunctions()
{
    ...
    {
		draw = draw || Sprite.prototype.draw;
		...
		if (draw.batched)
		{
			f.batched = function(context, index)
			{
				var previousAlpha = context.globalAlpha;
				this._f32RotationAlpha[1] = context.globalAlpha = alpha;
				draw.batched.call(this, context, index);
				context.globalAlpha = previousAlpha;
			};
		}
		return f;
    ...
    this.fadeOpacity_ = function(alphaStart, alphaEnd, time, originalDrawCall, callback)
    {
        ...
        var fun = function(context)
        {
            ...
            originalDrawCall.call(this, context);
            context.globalAlpha = previousAlpha;
            if (alpha == alphaEnd && (alpha == 1 || alpha == 0))
            {
		...
		if (originalDrawCall.batched)
		{
			fun.batched = function(context, index)
			{
				var oldAlpha = alpha;
				alpha = Math[alphaEnd > alphaStart? 'min' : 'max'](alphaEnd, alpha + alphaIncrement);
				var previousAlpha = context.globalAlpha;
				this._f32RotationAlpha[1] = context.globalAlpha = alpha;
				originalDrawCall.batched.call(this, context, index);
				context.globalAlpha = previousAlpha;
				if (alpha == alphaEnd && (alpha == 1 || alpha == 0))
				{
					(this.getDrawFunction() == fun) &&  this.setDrawFunction(originalDrawCall);
